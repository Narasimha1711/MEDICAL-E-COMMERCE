...
const sendRevenue = async (req, res) => {
    console.log("hi");
    const token = req.cookies.token1;
    if (!token) {
        return res.status(400).json({ message: "Login again", path: '/login' });
    ...
    jwt.verify(token, secret, {}, async (err, data) => {
        ...
        const totalRevenue = sellerDoc.medicinesUploaded.reduce((sum, item) => sum + (item.price * item.count), 0);
        const categoryWiseRevenue = {};
        sellerDoc.medicinesUploaded.forEach(item => {
            if (!categoryWiseRevenue[item.category]) {
                categoryWiseRevenue[item.category] = 0;
            }
            categoryWiseRevenue[item.category] += item.price * item.count;
        });
        return res.status(200).json({
            totalRevenue,
            categoryWiseRevenue,
            medicineCount: sellerDoc.medicinesUploaded.length
        });
    });
};

module.exports = { sellerSignup, sellerLogin, sellerInfo, sendRevenue };