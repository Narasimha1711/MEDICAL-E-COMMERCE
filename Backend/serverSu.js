...
//Multer Middleware (Third Party Middleware)
...
//   const { sellerID } = req.params;
//   try {
//     const seller = await SellerModel.findById(sellerID).populate("medicinesUploaded");
//     if (!seller || !seller.medicinesUploaded || seller.medicinesUploaded.length === 0) {
//       return res.status(404).json({ message: "No medicines found for this seller" });
//     }
//     const medicines = seller.medicinesUploaded;
//     // Simplified example: Calculate revenue for each day
//     const revenueData = [];
//     for (let i = 0; i < 30; i++) { // Example: Last 30 days
//       const date = new Date();
//       date.setDate(date.getDate() - i);
//       const dailyRevenue = medicines.reduce((acc, med) => {
//         if (new Date(med.createdAt).getDate() === date.getDate()) {
//           return acc + med.discountedPrice * med.count;
//         }
//         return acc;
//       }, 0);
//       revenueData.push({ date: date.toISOString(), revenue: dailyRevenue });
//     }
...
// app.get("/topMedicines/:sellerID", async (req, res) => {
//   const { sellerID } = req.params;
//   try {
//     const seller = await SellerModel.findById(sellerID).populate("medicinesUploaded");
//     if (!seller || !seller.medicinesUploaded || seller.medicinesUploaded.length === 0) {
...
//     }
//     const medicines = seller.medicinesUploaded;
//     // Simplified example: Sort by sales (count)
//     const sortedMedicines = medicines.sort((a, b) => b.count - a.count).slice(0, 5); // Top 5
//     const topMedicinesData = sortedMedicines.map(med => ({ name: med.medicine, sales: med.count }));
//     res.status(200).json(topMedicinesData);
//   } catch (error) {
//     console.error("Error fetching medicines:", error.message);
//     res.status(500).json({ message: "Internal server error" });
//   }
// });
// app.get("/revenueByCategory/:sellerID", async (req, res) => {
//   const { sellerID } = req.params;
//   try {
//     const seller = await SellerModel.findById(sellerID).populate("medicinesUploaded");
//     if (!seller || !seller.medicinesUploaded || seller.medicinesUploaded.length === 0) {
...
//     }
//     const medicines = seller.medicinesUploaded;
//     // Simplified example: Group by category and sum revenue
//     const revenueByCategory = {};
//     medicines.forEach(med => {
//       if (!revenueByCategory[med.category]) {
//         revenueByCategory[med.category] = 0;
//       }
//       revenueByCategory[med.category] += med.discountedPrice * med.count;
//     });
//     const categoryData = Object.keys(revenueByCategory).map(category => ({ category, revenue: revenueByCategory[category] }));
...
//   const { sellerID } = req.params;
//   try {
//     // Use MongoDB aggregation to get daily revenue
//     const revenueData = await OrderModel.aggregate([
//       { $match: { "products.seller": mongoose.Types.ObjectId(sellerID) } },
//       { $unwind: "$products" },
//       { $match: { "products.seller": mongoose.Types.ObjectId(sellerID) } },
//       {
//         $group: {
...
//           },
//           revenue: { 
//             $sum: { $multiply: ["$products.discountedPrice", "$products.count"] } 
//           }
//         }
//       },
//       { $sort: { "_id": 1 } },
...
//       const revenueResponse = await axios.get(`${API_BASE_URL}getRevenue/${sellerId}`);
...
//   const { sellerID } = req.params;
//   try {
//     // Use MongoDB aggregation to get top selling medicines
//     const topMedicines = await OrderModel.aggregate([
//       { $unwind: "$products" },
//       { $match: { "products.seller": mongoose.Types.ObjectId(sellerID) } },
//       {
//         $group: {
...
//   } catch (error) {
//     console.error("Error fetching top medicines:", error.message);
//     res.status(500).json({ message: "Internal server error" });
//   }
// });
// app.get("/topMedicines/:sellerID", async (req, res) => {
...
//   const { sellerID } = req.params;
//   try {
//     // Join orders with medicine data to get categories
...
// // ✅ Error-Handling Middleware (Place this at the end of `server.js`)
